...
{                                                                             }
{*****************************************************************************}
{ /lib/system/systeminfo.pp                                                   }
{ System Information Probes and Handlers                                      }
{*****************************************************************************}
{$H+}
...
function SystemGetRecord() : SystemInformation;
...
// The following are reserved functions (to be implemented in a later version)
...
{* Get the SystemInformation Record and return it as such *}
function SystemGetRecord() : SystemInformation;
  var
...
begin
     ...
     // SharkOS
     if fpstat('/etc/sharkos-release',info) = 0 then
     begin
       DetectedOS.ReleaseFile:='/etc/sharkos-release';
       DetectedOS.OSVendor := ParseTextLineVariable(GetTextLineBySubStr('/etc/sharkos-release','dist'), '=');
       DetectedOS.OSVersion := ParseTextLineVariable(GetTextLineBySubStr('/etc/sharkos-release','vers'), '=');
       DetectedOS.OSName := 'SharkOS v' + DetectedOS.OSVersion;
       exit(DetectedOS);
     end;
